...
-- 
-- The issue: When an activity is both scheduled AND cancelled in the same ack_orchestration_item
-- call (e.g., the future is dropped immediately after being scheduled), the worker item must
-- be inserted FIRST, then deleted by the cancellation. Previous ordering deleted first, then
-- inserted, causing the activity to remain in the queue.
--
-- Fix: Move cancelled_activities deletion to AFTER worker_items insertion.

DO $$
DECLARE
    v_schema_name TEXT := current_schema();
BEGIN
    -- Update ack_orchestration_item to fix cancellation ordering:
    -- 1. Insert worker items FIRST
    -- 2. THEN delete cancelled activities
    -- This ensures schedule-then-cancel pattern works correctly
