...
async fn sample_sub_orchestration_continue_as_new_fs() {
    ...
    let loop_child = |ctx: OrchestrationContext, input: String| async move {
        ...
        if iteration < 3 {
            return ctx.continue_as_new((iteration + 1).to_string()).await;
        }

        Ok(format!("child-final:{iteration}"))
    };
    let parent = |ctx: OrchestrationContext, input: String| async move {
        let result = ctx.schedule_sub_orchestration("LoopChild", input).await?;
    ...
    match client
        .wait_for_orchestration("inst-sub-can", std::time::Duration::from_secs(10))
        .await
        .unwrap()
    {
        runtime::OrchestrationStatus::Completed { output, .. } => {
            assert_eq!(output, "parent:child-final:3")
        }
        runtime::OrchestrationStatus::Failed { details, .. } => {
            panic!("orchestration failed: {}", details.display_message())
        }
...
/// Config hot-reload with persistent events (PostgreSQL version).
///
/// Demonstrates draining buffered events between work cycles using
/// `dequeue_event` + `select2` with a short timeout as a non-blocking drain.
///
        ...
        for cycle in 0..3 {
            ...
            loop {
                ...
                match ctx.select2(config_wait, drain_timeout).await {
                    ...
                    duroxide::Either2::Second(_) => break, // no more pending updates
                }
            }
