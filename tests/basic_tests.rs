...
async fn test_provider_creation_default_schema() {
    ...
    assert!(public_schema_exists_before, "Public schema should exist before cleanup");
    ...
    assert!(!table_exists_after, "Tables should be dropped from public schema");
    ...
    assert!(public_schema_exists_after, "Public schema must NEVER be dropped - it's a PostgreSQL system schema");
}
...
async fn test_read_empty_instance() {
    ...
    assert_eq!(events.len(), 0, "Reading non-existent instance should return empty");
...
async fn test_enqueue_orchestrator_work() {
    ...
    provider.enqueue_orchestrator_work(work_item, None)
        .await
        .expect("Failed to enqueue orchestrator work");
    
    // Verify instance was created
    let execution_id_opt = provider.latest_execution_id(instance_id).await;
    assert_eq!(execution_id_opt, Some(execution_id), "Instance should have execution_id");
...
async fn test_enqueue_and_dequeue_worker() {
    ...
    // Enqueue worker work
    provider.enqueue_worker_work(work_item.clone())
        .await
        .expect("Failed to enqueue worker work");
    
    // Dequeue worker work
    let (dequeued_item, lock_token) = provider.dequeue_worker_peek_lock()
        .await
        .expect("Should dequeue worker work");
    
    // Verify it's the same item
    match (&work_item, &dequeued_item) {
        (WorkItem::ActivityExecute { instance: i1, name: n1, .. },
         WorkItem::ActivityExecute { instance: i2, name: n2, .. }) => {
...
async fn test_management_capability() {
    ...
    let instances = mgmt.list_instances().await.expect("Failed to list instances");
    assert_eq!(instances.len(), 0, "Should return empty list for new schema");
    
    // Test system metrics
    let metrics = mgmt.get_system_metrics().await.expect("Failed to get metrics");
    ...
    // Test queue depths
    let queue_depths = mgmt.get_queue_depths().await.expect("Failed to get queue depths");
    assert_eq!(queue_depths.orchestrator_queue, 0, "Orchestrator queue should be empty");
...
async fn test_list_instances_and_executions() {
    ...
    provider.enqueue_orchestrator_work(work_item1, None).await.expect("Failed to enqueue");
    provider.enqueue_orchestrator_work(work_item2, None).await.expect("Failed to enqueue");
    
    // Test list_instances
    let mgmt = provider.as_management_capability().unwrap();
    let instances = mgmt.list_instances().await.expect("Failed to list instances");
    assert_eq!(instances.len(), 2, "Should have 2 instances");
    assert!(instances.contains(&instance1.to_string()), "Should contain instance1");
    assert!(instances.contains(&instance2.to_string()), "Should contain instance2");
    
    // Test list_executions
    let executions = mgmt.list_executions(instance1).await.expect("Failed to list executions");
